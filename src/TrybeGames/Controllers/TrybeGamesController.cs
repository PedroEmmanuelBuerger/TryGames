...
public class TrybeGamesController
{
    ...
    {
        var newPlayer = Console.ReadLine().ToString();
    ...
    public void AddGame()
    {
        var newGame = Console.ReadLine().ToString();
        var game = new Game();
        game.Id = database.Games.Count + 1;
        game.Name = newGame;
        var entrada = Console.ReadLine().ToString();
        var newD = DateTime.ParseExact(entrada, "dd/MM/yyyy", CultureInfo.InvariantCulture);
        game.ReleaseDate = newD;
        database.Games.Add(game);
        var gameTypeInput = Console.ReadLine().ToString();
         GameType newg = Enum.TryParse<GameType>(gameTypeInput, out GameType parsedGameType)
            ? parsedGameType
            : GameType.Other;
        game.GameType = newg;
